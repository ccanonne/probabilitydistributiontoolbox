...
\[
    \kolmogorov{p}{q} = \max_{i\in[\ab]} \abs{\sum_{j=1}^i p(i) - \sum_{j=1}^i q(i)} 
\] and
\[
...
$ where $\mathcal{C}$ is a class of measurable functions. For TV distance, $\mathcal{C}$ is the class of indicators of all measurable subsets; for Kolmogorov, this is the (smaller) class of indicators of intervals of the form $(-\infty, a]$. (For Wasserstein/EMD distance, this will be the class of continuous, $1$-Lipschitz functions.)
...
\end{theorem}
\begin{proof}[Sketch]
The proof for Kolmogorov distance is the most involved, and follows from a \emph{very} useful and non-elementary theorem due to Dvoretzky, Kiefer, and Wolfowitz from 1956 (with the optimal constant due to Massart, in 1990):
